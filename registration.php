...
public function register($fname, $lname, $sex, $email, $phone, $address, $password)
{
	...
	// Audit Trail
	$this->audittrail($phone, "reg", "success", $agent, "");
	
	// Update Vericode
	$updvericodeqry = sprintf("UPDATE `members` SET `authcode`= '$authcode' WHERE (`phone` = '$phone')");
	$runupd = mysqli_query($res, $updvericodeqry) or $this->ErrorLog("cmd=reg->".mysqli_error($res));
	if($runq) {	
		// SMS Validation for Mobile
		//$this->regauthSMS($authcode, $phone);
		$response = "OK";
	}
	else {
		mysqli_close($res);
...
function regauthMail($vericode, $email)
{
	...
	$message .= "Thank you for registering on ".$this->projectname." To complete your register please enter this verification code box <br />";
	$message .= "<br /> <b>Verification Code: </b>".$vericode;
	$message .= "<p></p>";
	$message .= "<p></p>";
	$message .= "<p></p>";
	$message .= "<p></p>";
	$message .= "<p>For questions and inquiries contact: info@myisusu.com or Call: 08062337154</p>";
...
function mailnotifier($tomem, $subj, $msg)
{
 /// Email Notifier ///         
	...
	$headers .= 'From: PhilHallmark <notify@myisusu.com>' . "\r\n";
...
// Check for Forgotten Password
{
	...
	mailnotifier($uemail, "Request for New Password", $msg);
}
// Check for Forgotten Password
//////////////////////////////////////////
